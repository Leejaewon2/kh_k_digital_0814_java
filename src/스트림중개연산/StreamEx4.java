...
2. 스트림 변환 :
- map() : 해당 스트림의 요소들을 함수에 인수로 전달하여, 그 반환값으로 이루어진 새로운 스트림을 만듬
- flatMap() : 각 요소를 변환하여 중첩된 스트림들을 평면화하여 새로운 스트림을 생성합니다.

3. 제한
- limit() : 해당 스트림의 첫 번째 요소부터 전달된 개수만큼 요소만으로 이루어진 새로운 스트림 생성
- skip() : 해당 스트림의 첫 번째 요솔부터 전달된 개수만큼의 요소를 제외한 나머지 요소만으로 이루어진 새로운 스트림 생성
...
//        stream4.flatMap(e->Stream.of(e.split(" +"))).forEach(e->System.out.print(e + " ")); // 출력값이 한번에 나옴
        // map과 flatMap 차이확인(???)
//        stream4.flatMap(e->Stream.of(e.split(" "))).forEach(System.out::println); // 출력값이 잘라서 나옴
